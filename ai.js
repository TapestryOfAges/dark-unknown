...
ais.Bandit = function(who, radius) {
  ...
  if ((who.getCurrentPath().length > 0) && (who.getTurnsToRecalcDest() > 0)) {
    ...
    if (GetDistance(who.getx(), who.gety(), coords[0], coords[1]) === 1) {  // the next step is only a step away
      ...
      if (diffx === 1) { retval = ais.Randomwalk(who,20,0,20,20); }
      else if (diffx === -1) { retval = ais.Randomwalk(who,20,20,20,0); }
      else if (diffy === 1) { retval = ais.Randomwalk(who,0,20,20,20); }
      else if (diffy === -1) { retval =  ais.Randomwalk(who,20,20,0,20); }
      else { alert("How did I get here? ais.Bandit."); }
